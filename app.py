...
def health_check():
    ...
    return jsonify({
        ...
        'mode': 'demo' if not OCR_AVAILABLE else 'production'
    }), 200

if __name__ == '__main__':
    # Get port from environment variable or default to 5000
    port = int(os.environ.get('PORT', 5000))
    host = '0.0.0.0'  # Listen on all interfaces for Render hosting
    debug = os.environ.get('FLASK_ENV', 'production') == 'development'
    
    print("\n" + "=" * 70)
    print(" " * 15 + "IMAGE TO TEXT EXTRACTION - OCR APPLICATION")
    print("=" * 70)
    print(f"\nMode: {'DEMO (OCR not available)' if not OCR_AVAILABLE else 'PRODUCTION (OCR available)'}")
